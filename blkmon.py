...
def new_block_event():
	try:
		...
		print "ERROR: failed to read", settings['pid'], ":", str(e)
		return
	
	m = re.search('(\d+)', line)
	if m is None:
		print "ERROR parsing", settings['pid']
		return
	
	pid = int(m.group(1))

	print "New block noticed, sending SIGUSR1 signal to PID", pid

	try:
		os.kill(pid, signal.SIGUSR1)
	except (OSError, IOError), e:
		print "sending SIGUSR1 failed:", str(e)
...
if __name__ == '__main__':
	...
	if 'pid' not in settings:
		settings['pid'] = '/var/lib/xf2/pool/pushpoold.pid'
